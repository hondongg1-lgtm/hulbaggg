    ...
    try {
      if (isSignUp) {
        const { data: authData, error: signUpError } = await supabase.auth.signUp({
          ...
          options: {
            data: {
              ...
              user_type: 'user' // Default to user on signup
            }
          }
        });
        ...
        if (authData.user) {
          ...
          onLoginSuccess(authData.user.id, 'user' as any);
        }
      } else {
        ...
        if (data.user) {
          // fetchUserProfile in AuthContext will handle the role detection
          // We just need to signal success
          onLoginSuccess(data.user.id, 'user' as any);
        }
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900 flex items-center justify-center p-4" dir="rtl">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-3xl font-black text-white mb-2">مرحباً بك</h1>
          <p className="text-emerald-200">سجل دخولك للوصول إلى لوحة التحكم</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <button
            ...
            disabled={loading}
            className="w-full px-6 py-3 bg-white border-2 border-slate-300 hover:border-emerald-500 hover:bg-emerald-50 text-slate-700 rounded-lg font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 mb-6 shadow-sm hover:shadow-md"
          >
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white text-slate-500 font-semibold">أو عبر البريد</span>
            </div>
          </div>
          ...
          <div className="mt-4 text-center">
            <a href="/" className="text-sm text-slate-600 hover:text-emerald-600 font-semibold transition-colors">
